"""ReservationAgent: the first specialist agent, handling reservation-domain tasks
end to end.

    ReservationAgent -> LLMProvider -> Qwen3-8B (via Ollama) -> typed Tools ->
    domain Services -> Repositories -> PostgreSQL

The agent never imports sqlalchemy, app.models, or app.repositories (enforced by
tests/unit/test_layer_boundaries.py) and never calls BGE-M3/an embedding provider —
this is tool-calling reasoning, not retrieval. It holds Tool instances (never
Service or Repository instances directly for reservation data) and a bounded ReAct
loop: ask the model for the next step, execute at most one round of tool calls,
feed results back, repeat until the model produces a final answer or the iteration
cap is hit.
"""
...
from app.agents.state import AgentErrorInfo, AgentState, ReservationAgentResult, ToolCallRecord
...
class ReservationAgent:
    ...
    async def handle(
        ...
        *,
        ...
        log.info("reservation_agent.started", task=task)
        ...
        try:
            ...
            log.error("reservation_agent.unexpected_failure", error=str(exc), exc_info=True)
        ...
        log.info("reservation_agent.finished", status=status, latency_ms=latency_ms, tool_calls=len(state.tool_calls))

        return ReservationAgentResult(
            agent_run_id=run.id,
            status=status,
            summary=summary,
    ...
    async def _execute_tool_call(
        ...
        *,
        ...
        if tool is None:
            ...
            log.warning("reservation_agent.unknown_tool", tool_name=call.name)
        else:
            try:
                output = await tool(call.arguments, context=context)
            except ToolError as exc:
                error = ToolErrorOutput.from_error(exc)
                log.warning("reservation_agent.tool_error", tool_name=call.name, code=exc.code)
            except ValidationError as exc:
                error = ToolErrorOutput(code="invalid_arguments", message=str(exc))
                log.warning("reservation_agent.invalid_arguments", tool_name=call.name)
            else:
